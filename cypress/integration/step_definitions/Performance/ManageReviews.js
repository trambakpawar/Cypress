...
When(/^Employee name should be displayed$/, function () {
    if (cy.contains("Lisa Andrews").should("be.visible")) {
        cy.screenshot()
    }
...
When(/^Employee name should be displayed on review page$/, function () {
    if (cy.contains("Garry White").should("be.visible")) { cy.screenshot() }
});

When(/^I click on evaluate button$/, function () {
    rv.evaluatebutton()
});
